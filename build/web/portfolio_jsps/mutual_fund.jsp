...
<html lang="en" dir="ltr">
    <head>
        <title>Mutual Funds Portfolio</title>
        ...
        <script>
            ...
            function validate()
            {
                ...
                var noFormat=/^[0-9]+$/  
                 ...
                 if((document.frm.mf_price.value.match(noFormat)==null))
                {
                    alert("Insert Correct price value.");
                    document.frm.mf_price.focus();
                    return false;
    ...
    <body>
        ...
        <div class="jspcontents">
            ...
            String q = "select * from mutualfund_info where uid='"+cuid+"'";
            rs = s.executeQuery(q);
        %>
        <div id="accordion">
            ...
            <div>
                ...
                <form name="frm" action="/Major_Project/AddMFServlet" method="get"  onsubmit="return validate();">
                    <table>
                        <th>Scheme Name</th>
                        <th>Quantity</th>
                        <th>Date</th>
                        ...
                        <tr>
                            <td>
                                <div id="searchQuotes" class="ticker-search mod" mode="search">
                                    ...
                                    <input required id="txtQuotes" name="mf_name" value="" type="text" autocomplete="off" class="yui-ac-input"/>

                                </div>
                            </td>
                    <td><input required type="text" name="quantity" /></td>
                    <td><input required type="text" class="tcal" name="date"/></td>
                    <td><input required type="text" name="mf_price" /></td>
                 ...
                 if(!rs.next()){
                     %>
                     <br/>
                 No Mutual Fund has been Purchased.
                 <%
                 }
                                 else{%>
            <h3>Edit Mutual Funds</h3>
            <div>

                <form method="post" name="form1" action="../portfolio_jsps/updatemutual_fund.jsp">
                    <table>
                        ...
                        <%
                            ...
                            rs.beforeFirst();
                            while (rs.next()) {
                                allsym1 = allsym1 + rs.getString("mf_name") + "+";
                        %>

                        <tr>
                            <td><input type="text" name="mf_name" readonly="readonly" value="<%= rs.getString("mf_name")%>" /></td>
                            ...
                            <td><input required type="text" name="mf_price" value="<%= rs.getString("mf_price")%>" onclick="this.value=''"/></td>
                            <input type="hidden" name="mf_id" value="<%= rs.getString("mf_id")%>" />
                            <td><input type="checkbox" name="delete" value="<%= rs.getString("mf_id")%>"/></td>
        ...
        <% rs.beforeFirst();
            if(rs.next()){
                 URL u1 = new URL("http://finance.yahoo.com/d/quotes.csv?s=" + allsym + "&f=d1l1c6n");
                ...
                while (rs.next()) {
                    ...
                    float i_p=Integer.parseInt(rs.getString("mf_price"));
                    total_inv=total_inv+(qu*i_p);
                   ...
                   float l_p1=Float.parseFloat(dataArray1[1]);
                    d_g1=d_g1+qu*ch1;
                    d_g2=df.format(d_g1);
                     o_g1=o_g1+((qu*l_p1)-(qu*i_p));
                     o_g2=df.format(o_g1);
                     latest_value1=latest_value1+(qu*l_p1);
